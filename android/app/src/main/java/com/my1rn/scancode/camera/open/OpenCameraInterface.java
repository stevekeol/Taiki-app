...
package com.my1rn.scancode.camera.open;
...
public final class OpenCameraInterface {
    ...
    public static OpenCamera open(int cameraId) {
        ...
        Camera camera;
        if (index < numCameras) {
            Log.d("Scancode", "Opening camera #" + index);
            camera = Camera.open(index);
        } else {
            if (explicitRequest) {
                Log.d("Scancode", "Requested camera does not exist: " + cameraId);
                camera = null;
            } else {
                Log.d("Scancode", "No camera facing " + CameraFacing.BACK + "; returning camera #0");
