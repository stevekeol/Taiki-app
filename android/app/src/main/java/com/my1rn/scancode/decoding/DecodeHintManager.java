...
public final class DecodeHintManager {
    ...
    static Map<DecodeHintType, ?> parseDecodeHints(Uri inputUri) {
        ...
        for (DecodeHintType hintType : DecodeHintType.values()) {
            ...
            if (hintType.getValueType().equals(int[].class)) {
                ...
                for (int i = 0; i < values.length; i++) {
                    try {
                        ...
                        Log.d("Scancode", "Skipping array of integers hint " + hintType + " due to invalid numeric value: '" + values[i] + '\'');
                        array = null;
                        break;
                    }
                }
            ...
            Log.d("Scancode", "Unsupported hint type '" + hintType + "' of type " + hintType.getValueType());
        }

        Log.d("Scancode", "Hints from the URI: " + hints);
        return hints;
    }
            ...
            if (extras.containsKey(hintName)) {
                if (hintType.getValueType().equals(Void.class)) {
                    ...
                    if (hintType.getValueType().isInstance(hintData)) {
                        ...
                        Log.d("Scancode", "Ignoring hint " + hintType + " because it is not assignable from " + hintData);
                    }
                }
            }
        }

        Log.d("Scancode", "Hints from the Intent: " + hints);
        return hints;
    }
